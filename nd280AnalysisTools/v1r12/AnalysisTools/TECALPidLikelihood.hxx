...
namespace ecalPid { // begin ND namespace
...
class EECALLikelihoodPdfManagerSetupFailure
    : public EECALPidLikelihoodException {
  virtual const char *what() const throw() {
...
class EECALLikelihoodAlgorithmMissingVariable
    : public EECALPidLikelihoodException {
  virtual const char *what() const throw() {
 ...
 *  @arg ecalRecon.Pid.Likelihood.PdfFile = v0000.0001 (specify the pdf file to
 *use)
 *
 ...
 *  @code
 *  //in initialisation
 *  ecalPid::TECALPidLikelihood* likelihoodCalculator = new
 *ecalPid::TECALPidLikelihood();
 *  likelihoodCalculator->SetupAlgorithms();//reads parameters file and sets up
 *defined algorithms.
 *
 *  //in the event loop
 *  ND::TTrackerECALReconModule::TECALReconObject& pidVarCalc = ... //get pid
 *var calc
...
public:
   ...
   *  ND::TTrackerECALReconModule::TECALReconObject::Process before calling this
   * method.
   *
   *  @param[in] inputVariables the object that contains the input variables.
   *
   */
  void CalculateLikelihoodValues(
      ND::TTrackerECALReconModule::TECALReconObject &inputVariables);
...
private:
  ...
  std::map<std::string, ecalPid::TECALPidLikelihoodAlgorithm *>
      fListOfAlgorithms;
  std::map<std::string, double> fLikelihoodRatioResults;
  TECALPidLikelihoodPdfManager *fPdfManager;

  typedef std::map<std::string, ecalPid::TECALPidLikelihoodAlgorithm
 ...
 * ND::TTrackerECALReconModule::TECALReconObject& inputVariables = ... //get pid
 * variable calculator
 * //you need to have calculated the variables by calling
 * ND::TTrackerECALReconModule::TECALReconObject::Process
...
public:
   ...
   *
   *	@param[in] algorithmName name of the algorithm, should be unique as it
   *will be used to name the TDatum when saving.
   *  @param[in] hypothesis1 sets particle type/classification to calculate
   *likelihoods.
   *  @param[in] hypothesis2 sets particle type/classification to calculate
   *likelihoods.
   *  @param[in] listOfInputVariables the variables/pdfs which will be combined
   *to form the log likelihood.
  ...
  /** @brief Destructor
   *
   * Cleans up the TECALPidLikelihoodHypothesisTest that this class wraps
   * around.
   */
  ~TECALPidLikelihoodAlgorithm();
   ...
   *
   * @param[in] inputVariables used to fetch the variable values. Hence
   * ND::TTrackerECALReconModule::TECALReconObject::Process must have been
   * called first to get the right answer.
   *
   * @return the value of the log-likelihood ratio
   */
  double CalculateLogLikelihoodRatio(
      ND::TTrackerECALReconModule::TECALReconObject &inputVariables);
...
private:
   ...
   *
   *  @param[in] pdfManager used to get the pdf histograms needed by this
   * algorithm.
   *
   */
   ...
   * @param[in] inputVariables the variable calculator object that stores
   * variable values.
   *
...
public:
   ...
   *  @return histogram.
   */
  TH1 *GetPdf(ecalPid::TECALPidDefs::PidHypothesisId_t hypothesis,
              ecalPid::TECALPidDefs::PidVariableId_t &variable, int dimensions);

private:
  struct PdfKey_t {
    PdfKey_t();
    PdfKey_t(const std::string &hypothesisName, const std::string &variableName,
             int dimensions);
   ...
   * 	@return histogram
   */
  TH1 *GetPdf(const std::string &hypothesisName,
              const std::string &variableName, int nDimensions);

  /** @brief parses the input histogram name to determine hypothesis and
   * variableName.
   *
   ...
   *
   */
  void ParseHistogramName(TH1 *hist, std::string &hypothesis,
                          std::string &variableName, int &dimensions) const;
  std::vector<std::string> TokenizeString(const std::string &inputText,
