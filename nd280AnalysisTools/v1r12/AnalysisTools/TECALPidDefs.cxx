...
std::string ecalPid::TECALPidDefs::GetVariableName(
    ecalPid::TECALPidDefs::PidVariableId_t variableId) const {
  std::map<PidVariableId_t, std::string>::const_iterator it =
      fVariableNames.find(variableId);
  if (it == fVariableNames.end()) {
    std::cerr << "TECALPidDefs::GetVariableName unknown variableId :"
              << variableId << std::endl;
...
ecalPid::TECALPidDefs::PidVariableId_t
ecalPid::TECALPidDefs::GetVariableId(const std::string &variableName) const {
  std::map<std::string, PidVariableId_t>::const_iterator it =
      fVariableIds.find(variableName);
  if (it == fVariableIds.end()) {
    std::cerr << "TECALPidDefs asked for variable ID for unknown variable:"
...
std::string ecalPid::TECALPidDefs::GetHypothesisName(
    ecalPid::TECALPidDefs::PidHypothesisId_t hypothesisId) const {
  std::map<PidHypothesisId_t, std::string>::const_iterator it =
      fHypothesisNames.find(hypothesisId);
  if (it == fHypothesisNames.end()) {
    std::cerr << "TECALPidDefs::GetHypothesisName unknown hypothesisId :"
              << hypothesisId << std::endl;
    throw EECALPidDefUnknownId();
  }
  return it->second;
}

ecalPid::TECALPidDefs::PidHypothesisId_t ecalPid::TECALPidDefs::GetHypothesisId(
    const std::string &hypothesisName) const {
  std::map<std::string, PidHypothesisId_t>::const_iterator it =
      fHypothesisIds.find(hypothesisName);
  if (it == fHypothesisIds.end()) {
    std::cerr
        << "TECALPidDefs asked for hypothesis ID for unknown hypothesis:\""
        << hypothesisName << "\"" << std::endl;
    throw EECALPidDefUnknownName();
  }
  return it->second;
}
...
void ecalPid::TECALPidDefs::InitializeVariables() {
  ...
  fVariableNames[kECALPidVar_ChargeStandardDeviation] =
      "ChargeStandardDeviation";
  ...
  fVariableNames[kECALPidVar_NormalizedMipChi2AllLayers] =
  ...
  for (std::map<PidVariableId_t, std::string>::iterator it =
           fVariableNames.begin();
       it != fVariableNames.end(); ++it) {
...
void ecalPid::TECALPidDefs::InitializeHypotheses() {
  ...
  // fill the inverted map
  for (std::map<PidHypothesisId_t, std::string>::iterator it =
           fHypothesisNames.begin();
       it != fHypothesisNames.end(); ++it) {
