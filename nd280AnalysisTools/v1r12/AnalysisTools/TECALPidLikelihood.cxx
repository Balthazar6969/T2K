...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
void ecalPid::TECALPidLikelihood::CalculateLikelihoodValues(
    ND::TTrackerECALReconModule::TECALReconObject &inputObject) {
  for (AlgorithmIterator it = fListOfAlgorithms.begin();
       it != fListOfAlgorithms.end(); ++it) {
    TECALPidLikelihoodAlgorithm *alg = it->second;
...
void ecalPid::TECALPidLikelihood::SetupAlgorithms() {
  ...
  ecalPid::TECALPidDefs::PidHypothesisId_t hypothesis1(
      ecalPid::TECALPidDefs::kECALPidHypothesis_Invalid);
  ...
  listOfInputVariables = std::vector<ecalPid::TECALPidDefs::PidVariableId_t>(
      varsTrackShower, varsTrackShower + nVars);
  alg = new ecalPid::TECALPidLikelihoodAlgorithm(
      algName, hypothesis1, hypothesis2, listOfInputVariables, *fPdfManager);
...
//	ecalPid::TOARuntimeParameters& runtimeParameters =
//ecalPid::TOARuntimeParameters::Get();
//	bool breakFlag = 0;
...
//			//get algorithm name
//			std::string algName =
//runtimeParameters.GetParameterS(parAlgName);
...
//			//an exception will be intentionally thrown if the
//parameters file is incorrectly configured
//			std::string hypothesis1Name =
//runtimeParameters.GetParameterS(parHypothesis1);
//			std::string hypothesis2Name =
//runtimeParameters.GetParameterS(parHypothesis2);
//			//convert string to PidHypothesisId_t enum.
//			ecalPid::TECALPidDefs::PidHypothesisId_t hypothesis1 =
//ecalPid::TECALPidDefs::Get().GetHypothesisId(hypothesis1Name);
//			ecalPid::TECALPidDefs::PidHypothesisId_t hypothesis2 =
//ecalPid::TECALPidDefs::Get().GetHypothesisId(hypothesis2Name);
//			//get the selected input variables
//			std::vector<ecalPid::TECALPidDefs::PidVariableId_t>
...
//				std::string parVariable = parInputVarRoot +
//jStr;
//				bool hasVariable =
//runtimeParameters.HasParameter(parVariable);
//				if (hasVariable)
//				{
//					//get variable name
//					std::string variableName =
//runtimeParameters.GetParameterS(parVariable);
//					//convert string to PidVariableId_t
//enum.
//					//an exception will be intentionally thrown if
//the variable is unknown.
//					ecalPid::TECALPidDefs::PidVariableId_t variable
//= ecalPid::TECALPidDefs::Get().GetVariableId(variableName);
//					listOfInputVariables.push_back(variable);
//				}
//				else if(j>1)
//				{
//					//couldn't find an variable, stop
//looking!
//					//note: we keep looking if j<=0 in case the user
//started from counting
//					//from 1.
//					inputVariablesBreakFlag = 1;//break out of the
//loop.
//				}
//			}
//			//check that pdf manager has been loaded
...
//			}
//			//we now have all of the information required to create
//the
//			//create a list of likelihood calculator
//			ecalPid::TECALPidLikelihoodAlgorithm* alg = new
//ecalPid::TECALPidLikelihoodAlgorithm(algName, hypothesis1, hypothesis2,
//listOfInputVariables, *fPdfManager);
//			alg->Print();
...
//			//note: we keep looking if i<=0 in case the user started
//from counting
//			//from 1.
//			breakFlag = 1;
//		}
...
ecalPid::TECALPidLikelihoodAlgorithm::TECALPidLikelihoodAlgorithm(
    ...
    ecalPid::TECALPidDefs::PidHypothesisId_t hypothesis1,
    ecalPid::TECALPidDefs::PidHypothesisId_t hypothesis2,
    const std::vector<ecalPid::TECALPidDefs::PidVariableId_t>
        &listOfInputVariables,
    TECALPidLikelihoodPdfManager &pdfManager)
    : fAlgorithmName(algorithmName), fHypothesis1(hypothesis1),
      fHypothesis2(hypothesis2), fListOfInputVariables(listOfInputVariables),
      fLikelihood(0) {
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
void ecalPid::TECALPidLikelihoodAlgorithm::Configure(
    TECALPidLikelihoodPdfManager &pdfManager) {
  fLikelihood = new TECALPidLikelihoodHypothesisTest(fAlgorithmName);
  ecalPid::TECALPidDefs::PidVariableId_t angleVariable =
      ecalPid::TECALPidDefs::kECALPidVar_Angle;
  // setup the likelihood hypothesis test
  for (std::vector<ecalPid::TECALPidDefs::PidVariableId_t>::iterator it =
           fListOfInputVariables.begin();
       it != fListOfInputVariables.end(); ++it) {
    ecalPid::TECALPidDefs::PidVariableId_t variable = *it;
    TH2F *histPdf1 =
        dynamic_cast<TH2F *>(pdfManager.GetPdf(fHypothesis1, variable, 2));
    fLikelihood->AddPdf(fHypothesis1, histPdf1, variable, angleVariable);
    TH2F *histPdf2 =
        dynamic_cast<TH2F *>(pdfManager.GetPdf(fHypothesis2, variable, 2));
    fLikelihood->AddPdf(fHypothesis2, histPdf2, variable, angleVariable);
  }
  return;
}
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
double ecalPid::TECALPidLikelihoodAlgorithm::CalculateLogLikelihoodRatio(
    ND::TTrackerECALReconModule::TECALReconObject &inputObject) {
  ...
  double logLikelihoodRatio = fLikelihood->GetLogLikelihoodRatio(
      this->fHypothesis1, this->fHypothesis2);
  return logLikelihoodRatio;
}
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
double ecalPid::TECALPidLikelihoodAlgorithm::GetVariableValue(
    ecalPid::TECALPidDefs::PidVariableId_t variableId,
    ND::TTrackerECALReconModule::TECALReconObject &inputObject) {
  //	double result = 0.;
  //	THandle<TRealDatum> datum =
  //inputObject->Get<TRealDatum>(variableName.c_str());
  //	if(datum)
  //	{
  ...
  //	{
  //		inputObject->ls();
  //		ND280Error("TECALPidLikelihoodAlgorithm cannot find variable
  //named:"<<variableName<<" inside input object.");
  ...
  default:
    std::cerr
        << "TECALPidLikelihoodAlgorithm::GetVariableValue unknown variable "
        << variableId << std::endl;
...
void ecalPid::TECALPidLikelihoodAlgorithm::Print() {
  std::string hyp1Name =
      ecalPid::TECALPidDefs::Get().GetHypothesisName(fHypothesis1);
  std::string hyp2Name =
      ecalPid::TECALPidDefs::Get().GetHypothesisName(fHypothesis2);
  std::cout << "TECALPidLikelihoodAlgorithm("
            << "algName=" << fAlgorithmName << ", hypothesis1=" << fHypothesis1
            << " (" << hyp1Name << ")"
  ...
  for (std::vector<ecalPid::TECALPidDefs::PidVariableId_t>::iterator it =
    ...
    std::cout << "variable" << i << "= " << varId << " (" << varId << ")"
              << std::endl;
    ++i;
  }
  return;
...
ecalPid::TECALPidLikelihoodPdfManager::~TECALPidLikelihoodPdfManager() {
  ...
  for (std::map<ecalPid::TECALPidLikelihoodPdfManager::PdfKey_t,
                TH1 *>::iterator it = fPdfs.begin();
       it != fPdfs.end(); ++it) {
    TH1 *hist = it->second;
    if (hist) {
      delete hist;
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
TH1 *ecalPid::TECALPidLikelihoodPdfManager::GetPdf(
    ecalPid::TECALPidDefs::PidHypothesisId_t hypothesis,
    ecalPid::TECALPidDefs::PidVariableId_t &variable, int nDimensions) {
  std::string hypothesisName =
      ecalPid::TECALPidDefs::Get().GetHypothesisName(hypothesis);
  std::string variableName =
      ecalPid::TECALPidDefs::Get().GetVariableName(variable);
  return this->GetPdf(hypothesisName, variableName, nDimensions);
}
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
TH1 *ecalPid::TECALPidLikelihoodPdfManager::GetPdf(
    const std::string &hypothesisName, const std::string &variableName,
    int nDimensions) {
  if (!fHasLoadedPdfs) {
    this->LoadPdfs();
  }
  TH1 *result = 0;
  ecalPid::TECALPidLikelihoodPdfManager::PdfKey_t key(
      hypothesisName, variableName, nDimensions);
  std::map<ecalPid::TECALPidLikelihoodPdfManager::PdfKey_t,
           TH1 *>::const_iterator it = fPdfs.find(key);
  if (it != fPdfs.end()) {
    result = it->second;
  }
  if (!result) {
    std::cerr
        << "TECALPidLikelihoodPdfManager unable to find pdf for hypothesis:"
        << hypothesisName << ", variable:" << variableName << std::endl;
    // print out names of all pdfs
    for (it = fPdfs.begin(); it != fPdfs.end(); ++it) {
      std::cerr << "pdfKey=" << it->first.info() << ", hist=" << it->second
                << std::endl;
    }
...
void ecalPid::TECALPidLikelihoodPdfManager::LoadPdfs() {
  ...
  if (pdfFile.IsOpen()) {
    ...
    TObject *keyObj = 0;
    while ((keyObj = next())) {
      // std::cout << "found key=" << keyObj->GetName() << ", class=" <<
      // keyObj->ClassName() << std::endl;
      TKey *key = dynamic_cast<TKey *>(keyObj);
      // std::cout << "found object=" << key->GetName() << ", class=" <<
      ...
      if (hist) {
        ...
        // fill container
        ecalPid::TECALPidLikelihoodPdfManager::PdfKey_t k(
            hypothesis, variableName, dimensions);
        fPdfs[k] = hist;
  ...
  if (!isSuccess) {
    std::cerr << "TECALPidLikelihoodPdfManager ECAL Likelihood PDF file failed "
                 "to open."
              << std::endl;
...
std::string ecalPid::TECALPidLikelihoodPdfManager::FindPdfFile() const {
  std::string result;
  const char *envEcalReconRoot = gSystem->Getenv("ND280ANALYSISTOOLSROOT");
  if (!envEcalReconRoot) {
    std::cerr << "TECALPidLikelihoodPdfManager ND280ANALYSISTOOLSROOT "
  ...
  std::string ecalReconDir = ecalReconRoot + "/" + ecalReconConfig;
  ...
  if (dirp) {
    ...
    // TODO : implement a method for the user to manually choose an input PDF
    // file.
    //        in ecalRecon this was dne through the parameters file. However,
    //        nd280AnalysisTools has no parameters file (that I know of).
    ...
    //		bool setInParameterFile =
    //ecalPid::TOARuntimeParameters::Get().HasParameter(parPdfFile);
    //		if (setInParameterFile)
    //		{
    //			std::string selectedFile =
    //ecalPid::TOARuntimeParameters::Get().GetParameterS(parPdfFile);
  ...
  if (result.empty()) {
    std::cerr << "TECALPidLikelihoodPdfManager ECAL Likelihood PDF file not "
                 "available:"
              << "  Did you run "
                 "${ND280ANALYSISTOOLSROOT}/AnalysisTools/"
                 "nd280AnalysisTools-get-likelihood-pdfs.sh ?"
              << std::endl;
    throw EECALLikelihoodPdfManagerSetupFailure();
  }
  return result;
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
void ecalPid::TECALPidLikelihoodPdfManager::ParseHistogramName(
    TH1 *hist, std::string &hypothesis, std::string &variableName,
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
std::vector<std::string> ecalPid::TECALPidLikelihoodPdfManager::TokenizeString(
    const std::string &text, const std::string &sep) const {
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
ecalPid::TECALPidLikelihoodPdfManager::PdfKey_t::PdfKey_t(
    const std::string &hyp, const std::string &var, int d)
    : hypothesisName(hyp), variableName(var), dimensions(d) {
  // nothing to do.
}
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
bool ecalPid::TECALPidLikelihoodPdfManager::PdfKey_t::
operator<(const ecalPid::TECALPidLikelihoodPdfManager::PdfKey_t &rhs) const {
...
std::string ecalPid::TECALPidLikelihoodPdfManager::PdfKey_t::info() const {
  std::stringstream ss;
  ss << "(" << hypothesisName << "," << variableName << "," << dimensions
     << ")";
  return ss.str();
}
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
