...
//********************************************************************
MyAnalysis::MyAnalysis(TTree *tree):AnalysisBase(tree){
//********************************************************************
...
//********************************************************************
  ...
  FillVar("run",    GetRun());
  FillVar("subrun", GetSubrun());
  FillVar("evt",    GetEvent());

  if (_HMN_track!=-1){
    FillVar("mu_mom",      GetMomentum(_HMN_track));
    FillVar("mu_costheta", GetDirection(_HMN_track)[2]);
    FillVar("ntpcs",GetNSegmentsInDet(_HMN_track,"TPC"));
    FillVar("nfgds",GetNSegmentsInDet(_HMN_track,"FGD"));
       
    int tpc= GetClosestTPC(_HMN_track);
    if (tpc>-1){      
      FillVar("mu_mom_tpc", GetTpcMomentum(_HMN_track,tpc));      
      FillVar("nhitstpc",   GetTpcNHits(_HMN_track,tpc));
      FillVar("sigmap_p",   GetMomentumRelErrorLocal(_HMN_track,tpc));
    }
    
  }

  if (FiducialCut("FGD1",GetPositionStart(_HM_track)))
    FillVar("vfgd", 1);
  else if (FiducialCut("FGD2",GetPositionStart(_HM_track)))
    FillVar("vfgd", 2);
  else 
    FillVar("vfgd", 0);
...
//********************************************************************
  ...
  // 0. Good spill
  if ((GetIsGoodSpill() && GetHasGoodDQ() ) || IsMC()){
    CutPassed(0);
  }  
  else
    return true;
  ...
  // 3. Fiducial cut
  if (_HM_track!=-1){
    if (FiducialCut("FGD",GetPositionStart(_HM_track))){
      CutPassed(3);
    }  
    else
      return true;
