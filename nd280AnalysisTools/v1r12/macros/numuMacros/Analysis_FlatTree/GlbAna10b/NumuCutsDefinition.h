...
class NumuCutsDefinition{
 ...
 public:
  ...
  void reset(int charge = -1, bool useTYPEBcuts=false){
    ...
    // ----------- tracks with TPC constituents---- ---------------
    for(int i = 0; i < NMAXTRACKS; i ++) scutin[i][0] = "NTracksWithTPC>0";
    
    // ------------- Track quality -------------------------------------
    for(int i = 0; i < NMAXTRACKS; i ++){
      stringstream out_itrk;out_itrk<<i;
      string s_itrk=out_itrk.str();
      quality[i] = "(Alt$(trTpcNHits["+s_itrk+"][0],0)+Alt$(trTpcNHits["+s_itrk+"][1],0)+Alt$(trTpcNHits["+s_itrk+"][2],0)>18)";
    ...
    for(int i = 0; i < NMAXTRACKS; i ++){
      stringstream out_itrk;out_itrk<<i;
      string s_itrk=out_itrk.str();
      scutin[i][2] = "( Bunch>0 && Bunch<7 && abs(Alt$(trPosStart["+s_itrk+"][3],0)-TBunch) < 4*21.5 )";
    }
    ...
    for(int i = 0; i < NMAXTRACKS; i ++){
      ...
      string s_itrk=out_itrk.str();
      if(i==0 && useTYPEBcuts)
        scutin[i][5] = "( (Alt$(trCharge[0],0) == -1) ||  (Alt$(trCharge[0],0)== 1 && Alt$(trCharge[1],0)==-1))"; 
      else
        scutin[i][5] = "(Alt$(trCharge["+s_itrk+"],0) == -1)";
      
      if(charge == 1) scutin[i][5] = "(Alt$(trCharge["+s_itrk+"],0) == 1)";
    }
    ...
    for(int i = 0; i < NMAXTRACKS; i ++){
      ...
      //uncorrected pulls
      if(useTYPEBcuts){
        pidqualityTPC[i][0] = "(( (abs(Alt$(trPullmu["+s_itrk+"][0],0))<4 || abs(Alt$(trPullele["+s_itrk+"][0],0))<4 || abs(Alt$(trPullp["+s_itrk+"][0],0))<4) && Alt$(trPullmu["+s_itrk+"][0],0)!=0) || Alt$(trDetUsed["+s_itrk+"][0],0)==0)"; 
        pidqualityTPC[i][1] = "(( (abs(Alt$(trPullmu["+s_itrk+"][1],0))<4 || abs(Alt$(trPullele["+s_itrk+"][1],0))<4 || abs(Alt$(trPullp["+s_itrk+"][1],0))<4) && Alt$(trPullmu["+s_itrk+"][1],0)!=0) || Alt$(trDetUsed["+s_itrk+"][1],0)==0)"; 
        pidqualityTPC[i][2] = "(( (abs(Alt$(trPullmu["+s_itrk+"][2],0))<4 || abs(Alt$(trPullele["+s_itrk+"][2],0))<4 || abs(Alt$(trPullp["+s_itrk+"][2],0))<4) && Alt$(trPullmu["+s_itrk+"][2],0)!=0) || Alt$(trDetUsed["+s_itrk+"][2],0)==0)"; 
      }
      else{
        pidqualityTPC[i][0] = "(( (abs(Alt$(g10aB_trPullCorrmu["+s_itrk+"][0],0))<4 || abs(Alt$(g10aB_trPullCorrele["+s_itrk+"][0],0))<4 || abs(Alt$(g10aB_trPullCorrp["+s_itrk+"][0],0))<4) && Alt$(g10aB_trPullCorrmu["+s_itrk+"][0],0)!=0) || Alt$(trDetUsed["+s_itrk+"][0],0)==0)"; 
        pidqualityTPC[i][1] = "(( (abs(Alt$(g10aB_trPullCorrmu["+s_itrk+"][1],0))<4 || abs(Alt$(g10aB_trPullCorrele["+s_itrk+"][1],0))<4 || abs(Alt$(g10aB_trPullCorrp["+s_itrk+"][1],0))<4) && Alt$(g10aB_trPullCorrmu["+s_itrk+"][1],0)!=0) || Alt$(trDetUsed["+s_itrk+"][1],0)==0)"; 
        pidqualityTPC[i][2] = "(( (abs(Alt$(g10aB_trPullCorrmu["+s_itrk+"][2],0))<4 || abs(Alt$(g10aB_trPullCorrele["+s_itrk+"][2],0))<4 || abs(Alt$(g10aB_trPullCorrp["+s_itrk+"][2],0))<4) && Alt$(g10aB_trPullCorrmu["+s_itrk+"][2],0)!=0) || Alt$(trDetUsed["+s_itrk+"][2],0)==0)"; 
      }
    ...
    for(int i = 0; i < NMAXTRACKS; i ++){
      ...
      string s_itrk=out_itrk.str();
      if(useTYPEBcuts){
        mupid[i] = " (( Alt$(trDetUsed["+s_itrk+"][0],0)==1 && abs(Alt$(trPullmu["+s_itrk+"][0],0))<2 && abs(Alt$(trPullele["+s_itrk+"][0],0))>2 ) || ( Alt$(trDetUsed["+s_itrk+"][1],0)==1 && abs(Alt$(trPullmu["+s_itrk+"][1],0))<2 && abs(Alt$(trPullele["+s_itrk+"][1],0))>2 ) || ( Alt$(trDetUsed["+s_itrk+"][2],0)==1 && abs(Alt$(trPullmu["+s_itrk+"][2],0))<2 && abs(Alt$(trPullele["+s_itrk+"][2],0))>2 )) ";
      }
      else{
    ...
    for(int i = 0; i < NMAXTRACKS; i ++){
      if(useTYPEBcuts && i==0)      
        scutin[i][7] = "( ( NTracksWithTPC>0 &&"+mupid[0]+") || ( NTracksWithTPC>=0 && "+mupid[1]+"&& Alt$(trCharge[0],0)==1 && Alt$(trCharge[1],0)==-1) )";
      else
        scutin[i][7] = "("+mupid[i]+")";
    ...
    // ------------- track multiplicity --------------------
    //    for(int i = 0; i < NMAXTRACKS; i ++) scutin[i][8] = "NTracksWithTPC<3";
    for(int i=0;i<NMAXTRACKS;i++)scutin[i][8]="NTracksWithTPC<3 ";
  
    // ------------- negative track multiplicity ----------
    for(int i = 0; i < NMAXTRACKS; i ++) scutin[i][9] = "NNegTracksWithTPC==1";

    // -------------fgd unused hits --------------------
    for(int i=0;i<NMAXTRACKS;i++)scutin[i][10]="( ("+ fgd1FV_cut[0]+" && sNFGDUnusedHits[0]<15 ) || ("+fgd2FV_cut[0]+" && sNFGDUnusedHits[1]<8))";

    // ------------- no michel electron -------------------
    for(int i = 0; i < NMAXTRACKS; i ++) scutin[i][11] = "NClusterscut[0][2]==0";

    // -------------- vertex cut. Make sure that both tracks have a common vertex ------------- 
    scutin[0][12] = "((abs(Alt$(trPosStart[0][0],0)-Alt$(trPosStart[1][0],0))<50 && abs(Alt$(trPosStart[0][1],0)-Alt$(trPosStart[1][1],0))<50 && abs(Alt$(trPosStart[0][2],0)-Alt$(trPosStart[1][2],0))<30 ) && " + scutin[1][2]+")";
    scutin[1][12] = "((abs(Alt$(trPosStart[0][0],0)-Alt$(trPosStart[1][0],0))<50 && abs(Alt$(trPosStart[0][1],0)-Alt$(trPosStart[1][1],0))<50 && abs(Alt$(trPosStart[0][2],0)-Alt$(trPosStart[1][2],0))<30 ) && " + scutin[0][2]+")";
    

    // ------------ Positive Track quality -------------------------
    scutin[0][13] ="("+ quality[1] + " &&  Alt$(trCharge[1],0)==1)";
    scutin[1][13] ="("+ quality[0] + " &&  Alt$(trCharge[0],0)==1)";

    // ------------ Positive PID quality -------------------------
    scutin[0][14] ="("+ pidquality[1] + " &&  Alt$(trCharge[1],0)==1)";
    scutin[1][14] ="("+ pidquality[0] + " &&  Alt$(trCharge[0],0)==1)";

    // ------------- proton PID  ---------------------
    ppid[0] = "(( Alt$(g10aB_trProbp[1],0)/(1- Alt$(g10aB_trProbk[1],0))>0.97)  &&  Alt$(trCharge[1],0)==1)";
    ppid[1] = "((g10aB_trProbp[0]/(1-g10aB_trProbk[0])>0.97)  && trCharge[0]==1)";

    // Most energetic positive track should be a proton
    for(int i = 0; i < NMAXTRACKS; i ++) scutin[i][15] = "(" + ppid[i] + ") ";

    // -------------- Delta_theta_p --------------------------------
    for(int i = 0; i < NMAXTRACKS; i ++) scutin[i][16] = "(DeltaThetaP[0]<0.6)";

    // -------------- Neutrino invariant mass ----------------------
    for(int i = 0; i < NMAXTRACKS; i ++) scutin[i][17] = "(NuInvMass>-0.4)";
  ...
  void DoCumulativeCut(){
	...
	scut[k][3] = scut[k][2] + " && ((("+scutin[0][3] +" || "+scutin[0][4] +") && ("+scut[0][2]+")) || (("+scutin[1][3] +" || "+scutin[1][4] +") && !("+scut[0][2]+")))" ;
      ...
      // vertex cut. Make sure that both tracks have a common vertex. Bunching 
      scut[k][11] = scut[k][10]  + " && (" + scutin[k][12] + " || NPosTracksWithTPC[0]==0)";
      // Track quality for 2nd track (>18 TPC hits) 
      scut[k][12] = scut[k][11] + " && (" + scutin[k][13] + " || NPosTracksWithTPC[0]==0)";
      // PID quality for 2nd track. at least one particle with pulls<4
      scut[k][13] = scut[k][12] + " && (" + scutin[k][14] + " || NPosTracksWithTPC[0]==0)";
      // Most energetic positive track should be a proton (positive track PID)
      scut[k][14] = scut[k][13] + " && (" + scutin[k][15] + " || NPosTracksWithTPC[0]==0)";
      ippid = 14;
      // Delta_theta_p
      scut[k][15] = scut[k][14] + " && (" + scutin[k][16] + " || NPosTracksWithTPC[0]==0)";
      // Neutrino invariant mass
      scut[k][16] = scut[k][15] + " && (" + scutin[k][17] + " || NPosTracksWithTPC[0]==0)";
      icutCCQE = 16;
    }
